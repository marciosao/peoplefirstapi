...
namespace PeopleFirst.Application.Services
{
    public class TipoFeedbackAppService : ITipoFeedbackAppService
    {
        ...
        public async Task<PagedResult<TipoFeedbackDto>> ListarPaginadoAsync(int page, int pageSize)
        {
            var total = await _repository.ContarAsync();
            var tipos = await _repository.ListarPaginadoAsync(page, pageSize);

            var dtos = tipos.Select(t => new TipoFeedbackDto
            {
                Id = t.Id,
                Tipo = t.Tipo
            });

            return new PagedResult<TipoFeedbackDto>(dtos, total, page, pageSize);
        }
